...
ssh "${REMOTE_HOST}" "
  ...
  docker build -t karaoke-api:latest -f infra/docker/api.Dockerfile .

  echo '==> Сборка образа WEB'
  docker build -t karaoke-web:latest \
    --build-arg VITE_APP_BASE=/karaoke/ \
    --build-arg VITE_API_BASE=/karaoke/api \
    -f infra/docker/web.Dockerfile .

  echo '==> Перезапуск API'
  docker rm -f karaoke-api >/dev/null 2>&1 || true
  docker run -d \
    --name karaoke-api \
  ...
  echo '==> Перезапуск WEB'
  docker rm -f karaoke-web >/dev/null 2>&1 || true
  docker run -d \
    --name karaoke-web \
    --restart unless-stopped \
    --network karaoke-net \
    -e VITE_ALLOWED_HOSTS=calc1.printninjas.ru \
  ...
  echo '==> Контейнеры'
  docker ps --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}' | grep karaoke
"
